...
function UIElement(x, y, width, height, type, ref, subref, slotType) {
  ...
  function Abacus(parentDivId, type) {
    ...
    var beadColor = "rgba(229, 109, 57, 1.0)";
    var hooveredBeadColor = "rgba(235, 172, 95, 1.0)";
    ...
    function drawBead(nodeId, ctx) {
        ...
        drawRoundRectFilled(ctx, dn.x+4, dn.y+4, dn.x2-dn.x, dn.y2-dn.y, 40);
        ctx.fillStyle = beadColor;
       
        if(nodeId === hooveredBead) {
          ctx.fillStyle=hooveredBeadColor;
        } 
        drawRoundRectFilled(ctx, dn.x, dn.y, dn.x2-dn.x, dn.y2-dn.y, 40);
      ...
      if (false) {
        ...
        ctx.fillText("ID: " + nodeId, dn.x + 4, dn.y2 - 35);
        ctx.lineWidth = 1;
      }
    }
